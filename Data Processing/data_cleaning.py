...
# Filter for Oxfordshire data only
...
df_prices = df_prices[df_prices['Ward name'].notna() & (df_prices['Ward name'] != '')].copy()
...
df_prices.to_csv(prices_output, index=False)
...
# Remove rows with all NaN values from the filtered dataset
df_broadband_filtered = df_broadband_filtered.dropna(how='all')

# Keep the ward codes to use in sql queries
df_broadband_filtered = df_broadband_filtered.merge(
